...
export default async function BeritaIndexPage() {
  const [beritaList, categories] = await Promise.all([
    getBeritaList({ status: 'terbit', orderBy: 'tanggal_publish', ascending: false }),
    getKategoriList(),
  ]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="pb-8 border-b border-[#E6E6E6] space-y-3">
            ...
            <p className="text-sm text-[#696969] max-w-2xl">
              Kumpulan artikel lengkap mengenai wisata, kelezatan kuliner, kesenian budaya, dan agenda terbaru di Kota Surakarta.
            </p>
          ...
          <div className="pt-8">
            {beritaList.length > 0 ? (
              ...
              <div className="bg-white border border-dashed border-gray-300 rounded-2xl p-12 text-center max-w-md mx-auto space-y-3">
                <p className="text-base font-semibold text-[#1D1D1D]">Belum ada berita</p>
                <p className="text-xs text-[#696969]">
                  Belum terdapat berita yang diterbitkan saat ini.
                </p>
              </div>
            )}
          </div>
